...
<script type="text/javascript">
	...
	function list(page){
		location.href = "itemboard?curPage="+page;
	}
/* 9-15 */
</script>
   ...
   <table class="itemboard"
    ...
    <tr>
      ...
      <th scope="col">추천수</th>
		...
		<!-- 공지글 -->
		<c:forEach var="notice" items="${map.itemNotice}">
			<tr>
				<td>공지</td>
				<td>
					<a href="<c:url value="/itemContent/${notice.boardNumber}" />" id="notice" class="notice">
			...
			<c:choose>
				<c:when test="${fn:contains(list.notice, 'n')}">
					<tr>
						<td>${list.rn}</td>
						<td>
							<a href="<c:url value="/itemContent/${list.boardNumber}" />">
