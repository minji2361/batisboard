...
public class PetChatController {
	...
	public ModelAndView petchatBoard(Model model,BoardVo boardVo,
		...
		String type="petchat";
		List<BoardVo> petchatNotice = boardService.noticeList(type);
		//session.setAttribute("petchatNotice", petchatNotice);
		...
		map.put("petchatNotice", petchatNotice);
		map.put("count",count);
...
//			likeCountVo.setMemberNumber(memberNumber);
		...
		//댓글 리스트 출력
				@ResponseBody
				@RequestMapping(value = "/petchatgetReplyList", method = RequestMethod.POST)
				...
				@ResponseBody
				@RequestMapping(value = "/petchatsaveReply", method = RequestMethod.POST)
			...
			@ResponseBody
			@RequestMapping(value = "/petchatupdateReply")
			...
			@ResponseBody
			@RequestMapping(value = "/petchatdeleteReply")
